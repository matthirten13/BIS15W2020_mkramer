...
#your code is fine, just another option
#names(fisheries)
```
...
```{r}
str(fisheries)
#again, your code is fine, just giving options - glimpse() wil also do this
```
...
#Other variables you might want to change the class of:
...
##always a good idea to sanity check
```{r}
#glimpse(fisheries)
```
...
fisheries2 <- fisheries %>% 
  dplyr::rename(country = "Country",
                ...
                unit = "Measure")
fisheries2
#a more efficient sanity check that doesn't print the whole dataset could be-
#names(fisheries2)
```
...
#This questions was asking for species of cephalopod (from question 14) so you would want to do your initial cephalopodsand years filtering again
```{r}
fisheries_tidy2 %>% 
...
##To show the taxonomic composition we might want to add code which would grou (group_by()) the output by species (AFSIS_sciname). Also we're already working in a dataframe so we don't need to redesginate it as such.
```{r}
