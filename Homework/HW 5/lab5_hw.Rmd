...
```{r}
amniota2 %>% 
  summarize(nas_amni = sum(is.na(amniota2))) #how many NAs
```
...
##consider making a data table to go along with your bar plot
```{r}
amniota_na %>% 
  count(class, sort = T)
```
...
#Note that you don't have to make a new object (amniota_gen) to answer this question. You can simply use the amniota2 dataset. This goes for #8 as well. Of course what you've done works just fine.
```{r}
amniota_gen <- amniota2 %>% 
  group_by(class) %>% 
  summarize(n_genera = n_unique(genus)) #n_distinct works here as well
amniota_gen
```
...
amniota_spec <- amniota2 %>% 
  group_by(class) %>% 
  summarize(n_species = n_unique(species))
amniota_spec
```

#Which means that your n_species variable is actually a count of genera here-
```{r}
amniota_spec %>% 
...
#Your code work fine, but note this can also be done using case_when(). See Dr. Ledofrd's example in his repository
...
#Using quartile ranges, I like it! You could also visually trim after looking at your scatter plot.
```{r}
